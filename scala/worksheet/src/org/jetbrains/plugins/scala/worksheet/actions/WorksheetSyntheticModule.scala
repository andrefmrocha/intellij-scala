...
final class WorksheetSyntheticModule(
  ...
  override def createError(error: Throwable, pluginId: PluginId): RuntimeException =
    cpModule.createError(error, pluginId)

  override def createError(message: String, pluginId: PluginId): RuntimeException =
    cpModule.createError(message, pluginId)

  override def createError(message: String, pluginId: PluginId, attachments: util.Map[String, String]): RuntimeException =
    cpModule.createError(message, pluginId, attachments)

  override def loadClass[T](className: String, pluginDescriptor: PluginDescriptor): Class[T] =
    cpModule.loadClass(className, pluginDescriptor)
}
